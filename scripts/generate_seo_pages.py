...
<style>
  .store-hero{{ padding:16px 0 4px; position:relative; z-index:1; }}
  ...
  .store-intro{{ font-size:14px; line-height:1.6; color:var(--ink-soft); max-width:64ch; margin:8px 0 36px; }}
  ...
  .card-top{{ display:flex; align-items:center; justify-content:center; gap:6px; margin-bottom:10px; }}
  .provider-badge{{ width:20px; height:20px; border-radius:6px; display:flex; align-items:center; justify-content:center; font-size:9.5px; font-weight:600; color:#fff; flex-shrink:0; }}
  .provider-name{{ font-size:13px; font-weight:500; }}
  .rate{{ font-family:'IBM Plex Mono', monospace; font-size:26px; font-weight:600; color:var(--ink); line-height:1; }}
  .card.best .rate{{ color:var(--lav-deep); }}
  .rate-label{{ font-size:10px; font-weight:600; letter-spacing:0.06em; text-transform:uppercase; color:var(--ink-soft); margin-top:3px; }}
  .card.best .rate-label{{ color:var(--lav-deep); }}
  .provider-meta{{ font-size:11px; color:var(--ink-soft); margin-top:8px; margin-bottom:12px; }}
...
<div class="wrap">
  ...
  <nav aria-label="Breadcrumb" style="font-size:12.5px; color:var(--ink-soft); margin-top:8px;">
  ...
  <p class="store-intro">Compare {store_name} cash back across {provider_list}. Rates change often - activate the offer before you check out to make sure it tracks.</p>

  <div class="store-faq">
    <h2>{store_name} cash back FAQ</h2>
    <div class="faq-item">
      <h3>How do I get {store_name} cash back?</h3>
    ...
    <div class="faq-item">
      <h3>Which site pays the most for {store_name}?</h3>
